...
Un caso distinto de los tres anteriores es **"Demanda agregada"**, cuarto eslabón del mecanismo ilustrativo de §5.2. A diferencia de "expectativas" o de la tasa de interés del Banco Central, esta variable no figuraba en ninguna celda "controla"/"recibe" de la tabla anterior — no por tratarse de un supuesto, sino porque es una **identidad macroeconómica** (demanda agregada = consumo + inversión + gasto público + exportaciones netas), resultante de decisiones de varios agentes a la vez (Hogares, Empresas, Gobierno y, cuando está activo, Sector externo — §7.2, Mundell-Fleming), no una variable que un único agente controle o reciba. En el modelo IS-LM base (cerrado, corto plazo, principio 1.2 y §7.1) el Sector externo permanece inactivo, de modo que la identidad se reduce a demanda agregada = consumo + inversión + gasto público. Se incorpora al catálogo como variable `demanda-agregada` (`tipo: resultado`, Doc. 2 §4.1; agente `empresas`, porque es la demanda agregada la que las Empresas reciben y ante la cual ajustan producción — el mismo rol causal que cumple como antecedente de "Producción" en el mecanismo de §5.2), agregada en Sprint 3 al detectarse la ausencia durante el Sprint Planning correspondiente. Ver `bitacora/sprint-03.md` §1–§2 y `ekg-macro/variables/demanda-agregada.yaml`.
...
| 1.4 | 2026-08-04 | §3: nueva nota bajo la tabla de agentes sobre **"Demanda agregada"** (cuarto eslabón del mecanismo de §5.2, ausente del catálogo real hasta ahora). Aclara que, a diferencia de "expectativas", no es un supuesto sino una identidad macro (consumo + inversión + gasto público + exportaciones netas, esta última inactiva en IS-LM base) resultante de varios agentes a la vez, sin agente único que la controle. Se incorpora al catálogo como variable `demanda-agregada` (`tipo: resultado`, agente `empresas`, por su rol de receptor causal en §5.2). Discrepancia detectada en Sprint Planning de S3: la variable no existía en el catálogo aceptado como "Hecho" en S2 pese a ser parte del ejemplo canónico de §5.2. Resuelta con definición económica del PO (identidad C+I+G+X-M, distinta de "demanda esperada"). Ver `bitacora/sprint-03.md` §1–§2. |
