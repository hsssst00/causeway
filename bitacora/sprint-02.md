...
|---|---|---|---|---|---|
| 1 | Esquema JSON de `variable` (nombre, agente(s), tipo, naturaleza, modelos_validos) + extensión de `reglas-integridad.js` con detección de variables duplicadas | E0 | Doc. 2 §4.3, Doc. 4 §7 (criterio de aceptación de la historia de catálogo de E2) | F0 | Hecho (Sprint Review 2026-08-04) |
| 2 | Catálogo completo de los seis agentes (Doc. 2 §3) y sus variables típicas (Doc. 2 §4.3), autorado como solución puente dentro de `causeway` | E2 | Doc. 2 §3, §4.3; Doc. 4 §7; Doc. 5 §3 | F0 | Hecho (Sprint Review 2026-08-04) |
...
## 6. Cierre del sprint — Sprint Review

**Fecha:** 2026-08-04
**Participantes:** PO + Claude, en sesión de chat (no Claude Code).
**Evidencia utilizada:** [`sprint-review/S2-evidencia.md`](../sprint-review/S2-evidencia.md), generado el mismo día — output real de `npm test` en `packages/cks/validate` (32/32), de `npm run validar` en `ekg-macro` (6/6 agentes, 25/25 variables, sin duplicados, sin inconsistencias agente↔variable) y demostración en vivo del rechazo de un catálogo sintético con una variable duplicada inyectada.

**Decisión del PO:** ambas historias de §2 se aceptan como **"Hecho"**, en vivo, durante la ceremonia. Esta aceptación es una decisión explícita del PO tomada en la Sprint Review contra el criterio de aceptación de cada historia (Doc. 4 §6.2: "Es demostrable en vivo en la Sprint Review correspondiente") — **no** un cierre automático porque CI esté en verde. CI en verde es condición necesaria (Doc. 4 §6.2, primer punto) pero no suficiente: la propia DoD exige además que sea demostrable en vivo y, para la historia 2, que el contenido económico esté confirmado por el PO. Ambas condiciones se satisficieron recién en esta ceremonia, no antes.

- **Historia 1:** aceptada sobre la base de los 32/32 tests de `packages/cks/validate` y la demostración en vivo de `variablesDuplicadas()` rechazando el catálogo sintético con la variable duplicada.
- **Historia 2:** aceptada sobre la base de 6/6 agentes y 25/25 variables válidos, sin duplicados ni inconsistencias agente↔variable, y de la firma económica firme de los cuatro puntos de clasificación del catálogo (incluida la reserva del punto 4 "Expectativas", cerrada por consulta externa el mismo día — ver §4, entrada "cierre de la reserva del punto 4").

**Salvedad sobre la evidencia escrita:** `sprint-review/S2-evidencia.md` se redactó el 2026-08-04 **antes** de que se cerrara la reserva del punto 4 — su sección final todavía describe la firma del punto 4 como "confirmación provisional", que ya no era el estado vigente al momento de esta ceremonia (la reserva se había cerrado horas antes — ver §4). El documento no se reescribe, por ser un registro histórico; queda anotado con una sección "Actualización posterior" que remite a esta entrada.
...
| 1.8 | 2026-08-04 | **Cierre formal de S2 tras Sprint Review.** El PO aceptó ambas historias de §2 como "Hecho" en vivo, en sesión de chat (PO + Claude), con base en `sprint-review/S2-evidencia.md`. §2: columna Estado de ambas historias pasa a "Hecho (Sprint Review 2026-08-04)". Nueva §6 "Cierre del sprint — Sprint Review": fecha, participantes, evidencia usada, y constancia explícita de que la aceptación es decisión del PO en la ceremonia (Doc. 4 §6.2), no cierre automático por CI verde; renumera la antigua §6 (Control de versiones) a §7. `sprint-review/S2-evidencia.md` anotado (no reescrito) con sección "Actualización posterior (2026-08-04)" que señala que quedó redactado antes del cierre de la reserva del punto 4 y remite a esta bitácora. Preparado `retro/sprint-02.md` como borrador (Estado: BORRADOR), con preguntas abiertas para el PO — sin completar "qué funcionó/qué no funcionó/acciones", a la espera de su reflexión. No se tocó `ekg-macro`, Doc. 2, Doc. 4 ni contenido de agentes/variables; no se abrió Sprint Planning de S3. **[Cerrada 2026-08-04 — ver `retro/sprint-02.md` v1.0.]** |
